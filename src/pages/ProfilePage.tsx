...
export function ProfilePage({ user, onBack, onAvatarChange }: ProfilePageProps) {
  ...
  const [tab, setTab] = useState<'profile' | 'friends' | 'search' | 'team'>('profile'); // ← 'team' добавлен
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex gap-1 bg-dark-200 rounded-xl p-1.5 overflow-x-auto">
          {([
            ...
            { id: 'team', label: 'Моя команда', icon: Shield },       // ← НОВАЯ ВКЛАДКА
          ...
          <div className="space-y-4">
            ...
            {!profile?.avatar_url && (
              <div className="card border-dashed border-dark-50 bg-transparent text-center py-6">
                ...
                <p className="text-gray-500 text-sm">Нажми на иконку камеры на аватаре, чтобы загрузить фото</p>
        ...
        {/* ── TEAM TAB (НОВОЕ) ── */}
        {tab === 'team' && (
          <TeamTab user={user} showToast={showToast} />
        )}
              ...
              <div>
                ...
                {friends.map(friend => (
                  <div key={friend.id} className="card flex items-center justify-between gap-3 mb-2">
                    ...
                    <button onClick={() => removeFriend(friend.id)} className="p-2 text-gray-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors" title="Удалить из друзей">
                  ...
                  <div key={req.id} className="card flex items-center justify-between gap-3 mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-gray-500 text-xs flex items-center gap-1"><Clock className="w-3 h-3" /> Ожидает подтверждения</div>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {searchResults.map(p => {
              ...
              return (
                <div key={p.id} className="card flex items-center justify-between gap-3">
                  ...
                  {status === 'friends' && (
                    <div className="flex items-center gap-1.5 px-3 py-1.5 bg-green-500/10 rounded-lg text-green-400 text-xs font-medium"><UserCheck className="w-3.5 h-3.5" /> Друг</div>
                  )}
                  {status === 'pending_sent' && (
                    <div className="flex items-center gap-1.5 px-3 py-1.5 bg-dark-200 rounded-lg text-gray-400 text-xs"><Clock className="w-3.5 h-3.5" /> Запрос отправлен</div>
                  )}
